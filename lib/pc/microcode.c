...
// clang-format off
const uint32_t microcode_updates [] = {
    /*
...
void cpu_update_microcode(void)
{
    uint32_t eax, ebx, ecx, edx;
    uint32_t pf = 0, sig, val[2];
    uint32_t x86_model, x86_family;
    struct microcode *m;
    int found = 0;

    ia32_platform_id_register platform_id_register;
    cpuid_eax_01 cpuid_info;

    cpu_write_msr(IA32_BIOS_SIGN_ID, 0, 0);
    cpu_read_cpuid(CPUID_VERSION_INFO, &eax, &ebx, &ecx, &edx);

    cpuid_info.eax.flags = eax;
    cpuid_info.ebx.flags = ebx;
    cpuid_info.ecx.flags = ecx;
    cpuid_info.edx.flags = edx;

    x86_model = cpuid_info.eax.model;
    x86_family = cpuid_info.eax.family_id;
    sig = cpuid_info.eax.flags;

    if ((x86_model >= 5) || (x86_family > 6)) {
        cpu_read_msr64(IA32_PLATFORM_ID, &platform_id_register.flags);
        pf = 1 << platform_id_register.platform_id;
        printf("[CPU] Platform ID2: %x %llx\n", pf,platform_id_register.flags);
    }
    ...
    if (found == 0) printf("[CPU] No Valid Microcode Update for this processor found\n");
}
